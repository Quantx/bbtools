...
impl Frame {
    fn to_array(&self, mirror: bool) -> [u8; LMT_FRAME_STRIDE] {
        ...
        let quat = Quat::from_euler(EulerRot::XYZ, self.rot.x, rot_y, rot_z);
...
impl Track {
    fn write(&self, mirror: bool, mut writer: impl Write) -> Result<usize, std::io::Error> {
        ...
        for frame in &self.frames {
            bytes_written += writer.write(&frame.to_array(mirror))?;
        }
    ...
    fn size(&self) -> usize {self.frames.len() * LMT_FRAME_STRIDE}
}
...
impl Animation {
    fn size(&self) -> usize {self.tracks.iter().fold(0, |acc, track| acc + track.size())}
}
...
impl LMT {
    pub fn import(buf: &[u8], scale : f32, fps : f32) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for animation in animations.iter_mut() {
            ...
            LittleEndian::read_u32_into(&animation_buf[..(track_count_max * 4)], &mut track_offsets);
            ...
            for track_offset in track_offsets {
                ...
                let (frame_list, []) = track_buf[4..frame_buf_end].as_chunks::<LMT_FRAME_SIZE>() else {
    ...
    pub fn node_count(&self) -> usize {self.track_count_max}
    
    pub fn size(&self) -> usize {self.animations.iter().fold(0, |acc, animation| acc + animation.size())}
    
    pub fn write_to_glbin(&mut self, path: &Path, include_mirror : bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn append_to_gltf(&self, root: &mut Root, glbin_rel_path: &Path, mirror_nodes: Option<&[u8]>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn append_to_gltf_mirror(&self, root: &mut Root, buf_idx: Index<Buffer>,  mirror_nodes: Option<&[u8]>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let view_name = if is_mirrored {"Keyframes Mirrored"} else {"Keyframes"};
...
impl fmt::Display for LMT {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln!(f, "Animation Count {}, Track Count Max {}",
            self.animations.len(), self.track_count_max)?;
        
        for (i, animation) in self.animations.iter().enumerate() {
            writeln!(f, "  Animation {:02}, Track Count {}, Duration {}",
                i, animation.tracks.len(), animation.duration)?;
        }
