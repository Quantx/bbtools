...
use crate::bbtools::obj::{MechModelConfig, ModelConfig};
...
pub struct Loadout {
    ...
    mounts: u8,
    presets: [u8; LOADOUT_SLOT_COUNT],
    fixed: [bool; LOADOUT_SLOT_COUNT],
}
...
impl Loadout {
    pub fn import(xbe: &mut XBE) -> Result<Vec<Self>, std::io::Error> {
        ...
        for i in 0..MECH_COUNT {
            ...
            let mut mweps: Vec<u8> = Vec::with_capacity(16);
            if mweps_pointer != 0 {
                xbe.seek_pointer_offset(mweps_pointer)?;
                xbe.reader.read_until(0xFE, &mut mweps)?;
            }
            mweps.retain(|&v| v < 0xFE);

            let mut sweps: Vec<u8> = Vec::with_capacity(16);
            if sweps_pointer != 0 {
                xbe.seek_pointer_offset(sweps_pointer)?;
                xbe.reader.read_until(0xFE, &mut sweps)?;
            }
            sweps.retain(|&v| v < 0xFE);
...
impl Mech {
    pub fn import(
        ...
        for data in engine_data {
            ...
            let mut modcfgs: [MechModelConfig; 4] = [mech_models[id]; _];
    ...
    pub fn export(
        ...
        get_efg_idx_path: FnIndexToPath,
    ) -> Result<(), std::io::Error> {
        ...
        mech_path.push(format!("Mech_{:02}.mech_scene", self.get_id()));
        {
            ...
            // Write Eye Effect
            let eye_effect_path = get_efg_idx_path(self.eye.idx as usize);
        ...
        mech_path.push(format!("{:02}.mech", self.get_id()));
        {
            let file = File::create(&mech_path)?;
            let mut writer = BufWriter::new(file);
            ...
            writer.write_u8(self.loadout.mweps.len() as u8)?;
            for &v in self.loadout.mweps.iter() {
                writer.write_u8(v)?;
            }

            writer.write_u8(self.loadout.sweps.len() as u8)?;
            for &v in self.loadout.sweps.iter() {
                writer.write_u8(v)?;
            }

            writer.write_u8(self.loadout.tanks.len() as u8)?;
            for &v in self.loadout.tanks.iter() {
                writer.write_u8(v)?;
            }

            for v in self.loadout.presets {
                writer.write_u8(v)?;
            }

            for v in self.loadout.fixed {
