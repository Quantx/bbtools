...
use crate::bbtools::eff::{SMOKE_TRAIL_SIZE, TRACER_TRAIL_SIZE, TrailEffect};
use crate::bbtools::obj::{ModelConfig, ModelConfigFlags};
use crate::bbtools::write_godot_option_path;
use crate::bbtools::xbe::XBE;
use crate::bbtools::{write_godot_path, write_pascal_option_string, write_pascal_string};
...
impl WeaponData {
    pub fn import(buf: &[u8; WEAPON_DATA_SIZE], scale: f32, fps: f32) -> Self {
        ...
        return WeaponData {
            ...
            damage_falloff: LittleEndian::read_f32(&buf[24..28]),
...
impl WeaponFile {
    pub fn import(path: &Path, scale: f32, fps: f32) -> Result<Self, std::io::Error> {
        ...
        let smoke_trails = TrailEffect::import_smoke_multiple(smoke_buf, 15, scale);
        let mut tracer_trails = TrailEffect::import_tracer_multiple(tracer_buf, 33, scale);
...
impl Weapon {
    ...
    pub fn export(
        ...
        ));
        {
            ...
            let mech_path_str = self
                .effects
                .mech
                .map(|idx| efg_idx_path_fn(idx as usize))
                .map(|path| path.into_os_string().into_string().ok())
                .flatten();
            write_pascal_option_string(mech_path_str.as_deref(), &mut writer)?;

            let charging_path_str = self
                .effects
                .charging
                .map(|idx| efg_idx_path_fn(idx as usize))
                .map(|path| path.into_os_string().into_string().ok())
                .flatten();
            write_pascal_option_string(charging_path_str.as_deref(), &mut writer)?;
            ...
            let smoke_path_str = self
                .effects
                .smoke
                .map(|idx| efg_idx_path_fn(idx as usize))
                .map(|path| path.into_os_string().into_string().ok())
                .flatten();
            write_pascal_option_string(smoke_path_str.as_deref(), &mut writer)?;

            let casing_path_str = self
                .effects
                .casing
                .map(|idx| efg_idx_path_fn(idx as usize))
                .map(|path| path.into_os_string().into_string().ok())
                .flatten();
            write_pascal_option_string(casing_path_str.as_deref(), &mut writer)?;

            writer.write_f32::<LittleEndian>(self.effects.lights_offset.x)?;
            writer.write_f32::<LittleEndian>(self.effects.lights_offset.y)?;
            writer.write_f32::<LittleEndian>(self.effects.lights_offset.z)?;
        ...
        weapon_path.push(format!("{:02}.weapon", self.get_id()));
        {
            ...
            writer.write_f32::<LittleEndian>(self.data.damage_falloff)?;
