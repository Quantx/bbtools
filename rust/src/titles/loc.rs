...
pub fn unpack(
    ...
    /*** CONVERT ICONS ***/
    {
        ...
        let save_icon = XPR::open(&game_path)?;
        game_path.pop();
...

    {
        ...
        let title_icon = XPR::open(&game_path)?;
        game_path.pop();
    ...
    for mech in mechs.iter() {
        ...
        mech.export(&build_path, get_gltf_path, get_efg_idx_path)?;
    }
    ...
    */
    ...
    let _ = create_dir(&build_path);
    {
        ...
        for (cup, cut) in cockpit_uv_data {
            game_path.push(cup);
            let cockpit_uv = UV::import_cockpit(&game_path, cut, Vec2::new(512.0, 256.0))?;
            game_path.pop();

            build_path.push(cup);
            build_path.set_extension("spritesheet");
            cockpit_uv.export(&build_path, get_dds_path)?;
            build_path.pop();
        }
    }
    ...
    let _ = create_dir(&build_path);
    {
        ...
        let _ = create_dir(&build_path);
        {
            ...
            let lines_path = diff_paths(&build_path, build_base_path).unwrap();
            ...
            // Copy boot strings
            {
                ...
                strings_path = diff_paths(&build_path, build_base_path).unwrap();
            ...
            for g in 0..3 {
                ...
                font_path = diff_paths(&font_path, build_base_path).unwrap();
            ...
            let _ = create_dir(&build_path);
            {
                ...
                for (i, palette) in hud_palettes.iter().enumerate() {
                    ...
                    for color in palette {
                        let c = color.as_vec4() / 255.0;
                        writer.write_f32::<LittleEndian>(c.x)?;
                        writer.write_f32::<LittleEndian>(c.y)?;
                        writer.write_f32::<LittleEndian>(c.z)?;
                        writer.write_f32::<LittleEndian>(c.w)?;
                    }
                }
            }
            build_path.pop();
...

    {
        ...
        for mi in 0..GAME_MISSION_COUNT {
            ...
            let mut objects = OBJ::import_objects(&object_buf, GAME_SCALE);
            ...
            OBJ::apply_heightmap(&mut objects, &mission);
    ...
    for file_index in 0..bin_atari.file_count() {
        // PPD 0246 SWEP26 is bugged and should have a node count of 7, not 6
        let node_count_fix = if file_index == 246 { 1 } else { 0 };
    ...
    for file_index in 0..bin_model.file_count() {
        ...
        models.push(match XBO::import_scaled(&file_bytes[..], GAME_SCALE) {
            Err(e) => {
                eprintln!(
    ...
    for file_index in 758..764 {
        ...
        menu_models.push(match XBO::import_scaled(&file_bytes[..], GAME_SCALE) {
            Err(e) => {
                eprintln!("Failed to convert file {:04}, got error: {}", file_index, e);
                None
            }
