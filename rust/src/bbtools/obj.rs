...
const MODEL_CONFIG_SIZE: usize = 8;
#[derive(Copy, Clone)]
...
impl ModelConfig {
    pub fn import_a(buf: &[u8]) -> Self {
        assert!(buf.len() == MODEL_CONFIG_SIZE);
    ...
    pub fn import_pointers(
        ...
        let mut model_config_buf: [u8; MODEL_CONFIG_SIZE] = [0; _];
        for pointer in pointers {
            xbe.seek_pointer_offset(pointer)?;

            xbe.reader.read(&mut model_config_buf)?;

            let model_config = ModelConfig::import_a(model_config_buf.as_slice());
...
impl From<&[u8]> for MechModelConfig {
    fn from(buf: &[u8]) -> Self {
        ...
        return MechModelConfig {
            ...
            chassis: ModelConfig::import_a(&buf[2..10]),
            hatch: ModelConfig::import_a(&buf[10..18]),
        };
    }
}
...
impl ModelConfigFlags {
    pub fn import_a(buf: &[u8]) -> Self {
        ...
        return ModelConfigFlags {
            modcfg: ModelConfig::import_a(&buf[0..8]),
    ...
    pub fn import_b(buf: &[u8]) -> Self {
        assert!(buf.len() == MODEL_CONFIG_FLAGS_SIZE);
        return ModelConfigFlags {
            modcfg: ModelConfig::import_b(&buf[0..8]),
            flags: LittleEndian::read_u32(&buf[8..12]),
        };
    }
...
impl From<&[u8; OBJ_SIZE]> for OBJ {
    fn from(buf: &[u8; OBJ_SIZE]) -> Self {
        let zero: [u8; 48] = [0; _];
        assert!(&buf[48..OBJ_SIZE] == zero);

        return OBJ {
            life: LittleEndian::read_i16(&buf[0..2]),
            modcfg: ModelConfig::import_a(&buf[2..10]),
...
impl OBJ {
    pub fn import_objects(buf: &[u8], scale: f32) -> Vec<OBJ> {
        let (obj_list, []) = buf.as_chunks::<OBJ_SIZE>() else {
            panic!("buf length not a multiple of OBJ_SIZE");
        };
    ...
    pub fn apply_heightmap(objs: &mut [OBJ], mission: &Mission) {
        for obj in objs {
            if obj.flags & 1 == 0 // Don't place object on ground if this flag is set
    ...
    pub fn write_objects(
        ...
        for obj in objs {
            ...
            writer.write_f32::<LittleEndian>(obj.rotation.x)?;
            writer.write_f32::<LittleEndian>(obj.rotation.y)?;
            writer.write_f32::<LittleEndian>(obj.rotation.z)?;
        }
