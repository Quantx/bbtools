...
impl PPD {
    pub fn import(buf: &[u8], node_count_fix: usize) -> Result<Self, Box<dyn std::error::Error>> {
        Self::import_scaled(buf, 1.0, node_count_fix)
    }
    ...
    pub fn import_scaled(
        ...
        let node_count = if bone_offset > 0 {
            ...
            let mut valid_bone_count: usize = 0;
            for bone in bones_list {
                let bone_id = bone[0] as usize;
                let collider_id = bone[1] as usize;
                ...
                assert!(colliders[collider_id].bone.is_none());
                colliders[collider_id].bone = Some(bone[0]);

                valid_bone_count += 1;
            }

            assert!(valid_bone_count == collider_count);
