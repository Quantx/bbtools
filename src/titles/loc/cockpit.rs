...
use obj::{ModelConfig, ModelConfigFlags, POS_ROT_SIZE, PosRot};
use x86::X86Context;
use xbe::XBE;
...
impl Cockpit {
    pub fn import(
        ...
        for i in 0..COCKPIT_COUNT {
            let effect_offset = match i {
                0 | 3 => Vec3::new(0.0, 0.0, 0.01),
                _ => Vec3::new(0.0, 0.005, 0.01),
            };

            let mut indicators_lights = cockpit_indicator_lights[i];
            for indicator_light_opt in indicators_lights.iter_mut() {
                if let Some(indicator_light) = indicator_light_opt {
                    indicator_light.position += effect_offset;
                }
            }

            let mut eject_light = cockpit_eject_lights[i];
            eject_light.position += effect_offset;
    ...
    pub fn export(
        ...
        cockpit_path.push(format!("Cockpit_{}.cockpit_scene", self.id));
        {
            ...
            writer.write_u8(1)?;
            self.eject_light.write(efg_idx_path_fn, &mut writer)?;

            for indicator_light in self.indicators_lights.iter() {
                if let Some(light) = indicator_light {
                    writer.write_u8(1)?;
                    light.write(efg_idx_path_fn, &mut writer)?;
                } else {
                    writer.write_u8(0)?;
                }
            }
