...
impl Effect {
    ...
    pub fn write(
        ...
        writer.write_f32::<LittleEndian>(self.damping_delay)?;
...
impl UVFrame {
    ...
    pub fn import_display(buf: &[u8], texture_size: Vec2) -> Self {
        return UVFrame {
            start: Vec2::new(
                LittleEndian::read_f32(&buf[0..4]),
                LittleEndian::read_f32(&buf[4..8]),
...
impl UV {
    ...
    pub fn import_display(
        ...
        for _ in 0..frame_count {
            ...
            frames.push(UVFrame::import_display(frame_buf.as_slice(), texture_size));
        }
...
impl EffectPosition {
    ...
    pub fn write(
        ...
        let effect_path = efg_idx_path_fn(self.idx as usize);
        write_godot_path(&effect_path, writer)?;
        writer.write_f32::<LittleEndian>(self.position.x)?;
        writer.write_f32::<LittleEndian>(self.position.y)?;
        writer.write_f32::<LittleEndian>(self.position.z)?;
...
impl TrailEffect {
    pub fn import_smoke(buf: &[u8; SMOKE_TRAIL_SIZE], uv_idx: usize, scale: f32, fps: f32) -> Self {
    ...
    pub fn import_tracer(
        buf: &[u8; TRACER_TRAIL_SIZE],
        uv_idx: usize,
        scale: f32,
        fps: f32,
    ) -> Self {
    ...
    pub fn import_smoke_multiple(buf: &[u8], uv_idx: usize, scale: f32, fps: f32) -> Vec<Self> {
        ...
        for smoke_buf in smoke_list {
            let smoke = Self::import_smoke(smoke_buf, uv_idx, scale, fps);
    ...
    pub fn import_tracer_multiple(buf: &[u8], uv_idx: usize, scale: f32, fps: f32) -> Vec<Self> {
        ...
        for tracer_buf in tracer_list {
            let tracer = Self::import_tracer(tracer_buf, uv_idx, scale, fps);
    ...
    pub fn export(&self, path: &Path, uv_path_fn: FnIndexToPath) -> Result<(), std::io::Error> {
        ...
        writer.write_f32::<LittleEndian>(self.sample_interval)?;
