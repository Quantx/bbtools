...
fn read_utf16_string(reader: &mut impl Read) -> Result<String, std::io::Error> {
...
const TEXT_CC_MAGIC: [u8; 2] = *b"CC";
const TEXT_DD_MAGIC: [u8; 2] = *b"DD";

pub fn import_cc_text(cc_buf: &[u8]) -> Result<String, std::io::Error> {
    if cc_buf[0..2] != TEXT_CC_MAGIC {
        return Err(std::io::Error::other("BadMagicCC"));
    }

    let header_count = LittleEndian::read_u16(&cc_buf[2..4]) as usize;
    let header_len = header_count * 6;

    let dd_buf = &cc_buf[4 + header_len..];

    if dd_buf[0..2] != TEXT_DD_MAGIC {
        return Err(std::io::Error::other("BadMagicDD"));
    }

    let data_len = LittleEndian::read_u32(&dd_buf[4..8]) as usize;

    let mut data_buf = &dd_buf[8..8 + data_len];
    data_buf = &data_buf[2..]; // Skip string size since we compute that manually

    return read_utf16_string(&mut data_buf);
}
