...
impl<'a> LMT<'a> {
    ...
    pub fn set_loop_mode(
        &mut self,
        animaiton_index: usize,
        loop_mode: LoopMode,
    ) -> Result<(), Box<dyn std::error::Error>> {
        if animaiton_index >= self.animations.len() {
            return Err("InvalidAnimationIndex".into());
        }

        self.animations[animaiton_index].loop_mode = loop_mode;
    ...
    pub fn append_to_gltf(
        ...
        if keyframes_size == 0 {
            // No animation data, nothing to do
            return Ok(());
        }
        ...
        for (i, animation) in self.animations.iter().enumerate() {
            ...
            for (j, track) in animation.tracks.iter().enumerate() {
                ...
                let time_acc_idx = root.push(Accessor {
                    ...
                    byte_offset: Some(USize64(track_offset)),
                    count: frame_count,
                ...
                let pos_acc_idx = root.push(Accessor {
                    ...
                    buffer_view: Some(key_view_idx),
                    byte_offset: Some(USize64(track_offset + 4)),
                    count: frame_count,
                    component_type: Checked::Valid(GenericComponentType(ComponentType::F32)),
                    extensions: None,
                    extras: Extras::default(),
                ...
                let rot_acc_idx = root.push(Accessor {
                    ...
                    buffer_view: Some(key_view_idx),
                    byte_offset: Some(USize64(track_offset + 16)),
                    count: frame_count,
                    component_type: Checked::Valid(GenericComponentType(ComponentType::F32)),
                    extensions: None,
                    extras: Extras::default(),
            ...
            animation_extension.insert(
                String::from("bbtools"),
                json!({"mirrored": is_mirrored, "loop_mode": animation.loop_mode.as_u32()}),
            );
